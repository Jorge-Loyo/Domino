...
const Live = (() => {
  ...
  async function recuperarPartida() {
    try {
      ...
      if (partida) {
        ...
        if (
          partida.puntos1 >= partida.meta ||
          partida.puntos2 >= partida.meta
        ) {
          mostrarGanador();
        }
      }
  ...
  function mostrarMarcador() {
    ...
    document.getElementById("live-meta-display").textContent =
      partidaActual.meta;
    ...
    document.getElementById("round-label1").textContent =
      partidaActual.equipo1_nombre;
    document.getElementById("round-label2").textContent =
      partidaActual.equipo2_nombre;
  ...
  function actualizarMarcador() {
    ...
    const pct1 = Math.min(
      100,
      (partidaActual.puntos1 / partidaActual.meta) * 100,
    );
    const pct2 = Math.min(
      100,
      (partidaActual.puntos2 / partidaActual.meta) * 100,
    );
    document.getElementById("team1-progress").style.width = pct1 + "%";
    document.getElementById("team2-progress").style.width = pct2 + "%";

    document
      .getElementById("score-team1")
      .classList.toggle(
        "winning",
        partidaActual.puntos1 > partidaActual.puntos2,
      );
    document
      .getElementById("score-team2")
      .classList.toggle(
        "winning",
        partidaActual.puntos2 > partidaActual.puntos1,
      );
  }

  async function sumarRonda(esEmpate = false) {
    ...
    // Si no es empate explícito, debe haber puntos en al menos un equipo
    if (!esEmpate && pts1 === 0 && pts2 === 0) {
      UI.mostrarNotificacion("Agrega puntos o registra un empate", "warning");
      return;
    }

    // En un empate se ignoran los puntos y se registra 0-0
    const p1 = esEmpate ? 0 : pts1;
    const p2 = esEmpate ? 0 : pts2;

    try {
      partidaActual = await API.sumarRondaVivo(partidaActual.id, p1, p2);
      document.getElementById("round-pts1").value = "0";
      document.getElementById("round-pts2").value = "0";
      actualizarMarcador();
      renderizarRondas();
      if (esEmpate) {
        UI.mostrarNotificacion("Ronda empatada registrada");
      }

      if (
        partidaActual.puntos1 >= partidaActual.meta ||
        partidaActual.puntos2 >= partidaActual.meta
      ) {
        mostrarGanador();
      }
    } catch (err) {
  ...
  function renderizarRondas() {
    ...
    if (rondas.length === 0) {
      container.innerHTML =
        '<p style="text-align:center; color:#757575; padding:1rem;">Sin rondas todavía</p>';
      return;
    }

    let acum1 = 0,
      acum2 = 0;
    container.innerHTML = rondas
      .map((r, i) => {
        acum1 += r.pts1;
        acum2 += r.pts2;
        const esEmpate = r.pts1 === 0 && r.pts2 === 0;
        const detalle = esEmpate
          ? `<span class="round-tie">🤝 Empate</span> (${acum1} - ${acum2})`
          : `+${r.pts1} / +${r.pts2} (${acum1} - ${acum2})`;
        return `
                <div class="round-item">
                    <span class="round-num">Ronda ${i + 1}</span>
                    <span class="round-scores">${detalle}</span>
